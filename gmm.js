...
Hooks.once("init", function() {
	...
	// Replace direct global usage with namespaced / fallback references
	const ActorsRef = (typeof foundry !== "undefined" && foundry.documents?.collections?.Actors) ? foundry.documents.collections.Actors : (globalThis.Actors ?? undefined);
	const ItemsRef  = (typeof foundry !== "undefined" && foundry.documents?.collections?.Items)  ? foundry.documents.collections.Items  : (globalThis.Items  ?? undefined);
...
async function _hookActorDirectory(html) {
    // Normalize html (HTMLElement or jQuery) and guard
    ...
    try {
        ...
        // Guard: prefer the search header, fallback to the directory header, then to the root
        const dirSearch = root.querySelector(".directory-header .header-search");
        const dirHeader = dirSearch ?? root.querySelector(".directory-header") ?? root;
        if (dirHeader && dirHeader.parentNode) {
            dirHeader.parentNode.insertBefore(section, dirSearch ?? dirHeader);
        } else {
            // Fallback append to header or root if parentNode is missing
            (root.querySelector(".directory-header") || root).appendChild(section);
        }
    } catch (err) {
        console.warn("giffyglyph-monster-maker-continued | _hookActorDirectory failed", err);
        return;
    }
}
...
async function _hookItemDirectory(html) {
    ...
    try {
        ...
        console.warn("giffyglyph-monster-maker-continued | _hookItemDirectory failed", err);
        return;
    }
}
