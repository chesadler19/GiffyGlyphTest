...
const GmmItem = (function () {
    ...
    function patchItem5e() {
        const moduleId = "giffyglyph-monster-maker-continued";

        const tryRegister = (targetExpr, wrapperMethodName, mode = "WRAPPER") => {
            try {
                const targetExists = (new Function(`return !!(${targetExpr});`))();
                if (!targetExists) {
                    // Try alternate target for older/newer system shapes
                    if (targetExpr.includes("getAttackToHit") && CONFIG?.Item?.documentClass?.prototype?.getAttackToHit) {
                        targetExpr = "CONFIG.Item.documentClass.prototype.getAttackToHit";
                    } else {
                        console.warn(`${moduleId} libWrapper target not found: ${targetExpr}`);
                        return false;
                    }
                }

                const wrapperMember = this?.[wrapperMethodName];
                if (typeof wrapperMember === "function") {
                    libWrapper.register(moduleId, targetExpr, wrapperMember.bind(this), mode);
                    return true;
                }

                console.warn(`${moduleId} wrapper method missing: ${wrapperMethodName}`);
                return false;
            } catch (err) {
                console.warn(`${moduleId} libWrapper registration failed for ${targetExpr}`, err);
                return false;
            }
        };

        // Safer libWrapper registration: try several candidate targets and register the first that exists.
        const resolvePath = (path) => {
            return path.split('.').reduce((obj, key) => obj?.[key], globalThis);
        };

        const registerSafe = (candidates, wrapper, mode = "WRAPPER") => {
            const foundCandidate = candidates.find(candidate => {
                try { return !!resolvePath(candidate); } catch (e) { return false; }
            });
            if (!foundCandidate) {
                console.debug(`${moduleId} libWrapper target not found. candidates:`, candidates);
                return false;
            }
            try {
                libWrapper.register(moduleId, foundCandidate, wrapper, mode);
                return true;
            } catch (err) {
                console.warn(`${moduleId} libWrapper registration failed for ${foundCandidate}`, err);
                return false;
            }
        };

        // Example: attempt to wrap the attack-to-hit function using safe checks
        registerSafe([
            "game.dnd5e.documents.Item5e.prototype.getAttackToHit",
            "game.dnd5e.documents.Item5e.prototype.get5eAttackToHit",
            "CONFIG.Item.documentClass.prototype.getAttackToHit",
            "CONFIG.Item.documentClass.prototype.get5eAttackToHit"
        ...
        registerSafe([
            "game.dnd5e.documents.Item5e.prototype.prepareData",
        ...
        // register saveDC (include new name)
        registerSafe([
            "game.dnd5e.documents.Item5e.prototype.getSaveDC",
            "game.dnd5e.documents.Item5e.prototype.get5eSaveDC",
            "CONFIG.Item.documentClass.prototype.getSaveDC",
            "CONFIG.Item.documentClass.prototype.get5eSaveDC"
        ], function (wrapped, ...args) {
            if (this.getSheetId() == `${GMM_MODULE_TITLE}.ActionSheet`) {
        ...
        // register rollDamage (include new name)
        registerSafe([
            "game.dnd5e.documents.Item5e.prototype.rollDamage",
            "game.dnd5e.documents.Item5e.prototype.roll5eDamage",
            "CONFIG.Item.documentClass.prototype.rollDamage",
            "CONFIG.Item.documentClass.prototype.roll5eDamage"
        ], function (wrapped, ...args) {
        ...
        registerSafe([
            ...
            "game.dnd5e.documents.Item5e.prototype.use"
        ], function (wrapped, ...args) {
            if (this.getSheetId() == `${GMM_MODULE_TITLE}.ActionSheet` && this.isOwnedByGmmMonster()) {
        ...
        registerSafe([
            ...
            "game.dnd5e.documents.Item5e.prototype.displayCard"
        ], function (wrapped, ...args) {
            if (this.getSheetId() == `${GMM_MODULE_TITLE}.ActionSheet` && this.isOwnedByGmmMonster()) {
                const gmmMonster = this.getOwningGmmMonster();
                this.system.damage.parts = this.system.damage.parts.map((x) =>
        ...
        registerSafe([
            "game.dnd5e.documents.Item5e.prototype.rollFormula",
            "CONFIG.Item.documentClass.prototype.rollFormula"
        ], function (wrapped, ...args) {
            if (this.getSheetId() == `${GMM_MODULE_TITLE}.ActionSheet` && this.isOwnedByGmmMonster()) {
                return _rollActionDamage({
                    item: this,
